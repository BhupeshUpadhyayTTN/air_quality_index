...
def athena_query():
    ...
    day_wise_most_polluted_city = f"""SELECT city FROM (SELECT *, dense_rank() OVER(PARTITION BY 'day' ORDER BY 'average_pollutant_level') AS dense_ranking FROM {database_info['databaseName']}.{database_info['table']}) AS subquery WHERE dense_ranking < 4 limit 10;"""
    
    #Top 3 day wise most polluted state
    day_wise_most_polluted_state = f"""SELECT state FROM (SELECT *, dense_rank() OVER(PARTITION BY 'day' ORDER BY 'average_pollutant_level') AS dense_ranking FROM {database_info['databaseName']}.{database_info['table']}) AS subquery WHERE dense_ranking < 4 limit 10;"""
